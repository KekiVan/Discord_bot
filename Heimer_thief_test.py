...
class Music(commands.Cog):
    ...
    async def play_(self, ctx, *, search: str):
        ...
        if search.find("list=") != -1:
            ...
            for video in playlist_dict["entries"]:
                ...
                source = await YTDLSource.create_source(ctx, str(video.get("id")), loop=self.bot.loop,
                                                        download=False)
            ...
            try:
                ...
                embed = discord.Embed(title="", description=f'PRIVATE SHIT NGL',
                                      color=discord.Color.green())
                await ctx.send(embed=embed)
                return
    ...
    async def queue_info(self, ctx):
        ...
        if int(len(player.queue._queue))>=10:
            ...
            fmt = '\n'.join(
                f"`{(upcoming.index(_)) + 1}.` [{_['title']}]({_['webpage_url']}) | ` {duration} Requested by: {_['requester']}`\n"
                for _ in upcoming)
        fmt = f"\n__Now Playing__:\n[{vc.source.title}]({vc.source.web_url}) | ` {duration} Requested by: {vc.source.requester}`\n\n__Up Next:__\n" + fmt + f"\n**{int(len(player.queue._queue))} songs in queue**"
